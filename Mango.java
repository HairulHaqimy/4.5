...
public class Mango extends Fruits{
	...
	public Mango(String name, double g, double c, double p) {		
		super(name);	
		grams = g;
		Carbs = c;
		price = p;
		
		System.out.print("Please enter the grams of Mango : ");
		grams = input.nextDouble();
		
		if(grams < 200) {
			Carbs();
			System.out.println(name);
			System.out.println("The mango is LESS THAN 200g. \nTotal Carbohydrates : " + df2.format(Carbs()));
		} 
		else if(grams > 200 && grams < 400) {
			g = 346;
			Carbs(g);
			System.out.println(name);
			System.out.println("The mango is LESS THAN 200g and MORE THAN 400g. \nTotal Carbohydrates : " + df2.format(Carbs(g)));
		} 
		else if(grams > 400){
			int q = 3;
			double gg = 459;

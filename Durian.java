...
public class Durian extends Fruits{
	...
	public Durian(String name, double g, double c, double p) {		
		super(name);	
		grams = g;
		calories = c;
		price = p;
		
		System.out.print("Please enter the grams of durian : ");
		grams = input.nextDouble();
		
		if(grams < 200) {
			calories();
			System.out.println(name);
			System.out.println("The durian is LESS THAN 200g. \nTotal Calories: " + df2.format(calories()) + " kcal");
		} 
		else if(grams > 200 && grams < 400) {
			g = 235;
			calories(g);
			System.out.println(name);
			System.out.println("The durian is LESS THAN 200g and MORE THAN 400g. \nTotal Calories: " + df2.format(calories(g)) + " kcal");
		} 
		else if(grams > 400){
			int q = 5;
			double gg = 438;
			calories(gg, q);
			System.out.println(name);
			System.out.println("The durian is MORE THAN 400g. \nTotal Calories: " + df2.format(calories(gg, q)) + " kcal");
		} 
		else {
			System.out.println("Invalid input. Please enter a valid number.");
		}
		
		Discount bd = new MangoDiscount(); 
